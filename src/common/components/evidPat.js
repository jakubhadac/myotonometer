...
class Evid extends Component {
    constructor(props){
        ...
        this.btnSave = this.btnSave.bind(this); this.btnDelete = this.btnDelete.bind(this);
    }
    /* ... functions */
    getLastId(){
    ...
    btnSave(){
        ...
        if(this.props.pat.name === '' || this.props.pat.lastName === '' || this.props.pat.pin === ''){
            ...
            let p = this.createPatient();
            if(this.props.pat.id === -1 && this.props.pat.status === '') { let i = this.getLastId(); ws.patientAdd(i, p); }
            if( this.props.pat.id !== -1 && this.props.pat.status === ''){ ws.patientEdit(this.props.pat.id, p); }
        }
    }
    btnDelete(){
        if( this.props.pat.id !== -1 ){ ws.patientRemove( this.props.pat.id ); }
    }
    /* render */
    render(){
        return (
            <fieldset>
                ...
                <Row>
                    ...
                    <Col sm={3}>
                        ...
                        { this.props.pat.id !== -1 &&
                            <Button className="pull-right" onClick={this.btnDelete}>Delete</Button> }
...
export default connect(mapStateToProps, mapDispatchToProps)(Evid);